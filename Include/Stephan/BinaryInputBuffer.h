...
public:
	...
	explicit BinaryInputBuffer(const std::string& filename, bool preloadCompleteFile = false);
	...
	/// get a single bit
	BinaryInputBuffer& operator>>(bool& bit)
	{
		bit = getBool();
		return *this;
	}
	...
	/// get 8 bits
	BinaryInputBuffer& operator>>(unsigned char& byte)
	{
		byte = getByte();
		return *this;
	}
	...
	/// get 16 bits
	BinaryInputBuffer& operator>>(unsigned short& word)
	{
		word = getWord();
		return *this;
	}
	...
	/// get 32 bits
	BinaryInputBuffer& operator>>(unsigned int& dword)
	{
		dword = getInt();
		return *this;
	}
