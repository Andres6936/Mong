...
namespace Hash
{
	...
	/*unsigned int adler32(const void* data, size_t length, unsigned int previousAdler32)
	{
	  if (!data || length == 0)
		return previousAdler32;

	  const unsigned int AdlerModulo = 65521;
	  unsigned int a = previousAdler32 & 0x0000FFFF;
	  unsigned int b = previousAdler32 >> 16;

	  const unsigned char* current = (unsigned char*) data;

	  // unrolling
	  const unsigned int Unrolling = 16; // more than 16 doesn't seem to make it faster ...
	  while (length > Unrolling)
	  {
		a += *current++; b += a;
		a += *current++; b += a;
		a += *current++; b += a;
	...
	/// look-up table for sbox hash
	static const unsigned int crc32Lookup[8][256] =
			{
					//// generated by a standard CRC32 table maker (crc32Lookup[0]) ...
	...
	/// compute CRC32 based on polynom 0xEDB88320L (zlib's)
	unsigned int crc32(const void* data, size_t length, unsigned int previousCrc32)
	{
		const unsigned int* current = (unsigned int*)data;
